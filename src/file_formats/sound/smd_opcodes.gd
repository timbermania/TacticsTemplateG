...
## SMD / FEDS opcode definitions and track decoder.
##
## FFT music sequences (SMD) and effect-sound channels (FEDS) share one
## bytecode language. This decodes a raw channel/track byte stream into a
## flat list of NoteEvent and OpcodeEvent objects.
##
## Ported from the godot-learning project (addons/exmateria_sound/runtime/
## smd_opcodes.gd) so this project can parse sound data natively from the ISO.
## Reference: https://ffhacktics.com/wiki/Sound_Novel (sound section).

# Tick durations indexed by (data_byte % 19). Index 0 means "read the next
# byte as an explicit duration".
const DELTA_TIME_TABLE: PackedInt32Array = [0, 192, 144, 96, 72, 64, 48, 36, 32, 24, 18, 16, 12, 9, 8, 6, 4, 3, 2]

const PULSES_PER_QUARTER_NOTE: int = 48

# opcode -> [name, parameter_count]
const OPCODE_INFO: Dictionary = {
	...
	0xAC: ["Instrument", 1],
	0xAD: ["Byte76_Adjust", 1],             # slot+0x76 sign-extended add
	...
	0xE6: ["LFO_SubSlot1_Activate", 0],     # set chan+0x11E bit 0x1
...
# Opcodes with known parameter counts but unimplemented at the dispatcher
# level — they consume the correct number of parameter bytes so the byte
# stream does not desync. Parameter counts derived from the FFT size table at
# ROM 0x80028d0c (table value - 1).
const EXTRA_OPCODE_PARAM_COUNTS: Dictionary = {
...
class NoteEvent:
	...
	var delta_time: int
	var note_byte: int     # raw bytecode 2nd byte; FFT chan+0x92 source
...
## Returns an Array of NoteEvent and OpcodeEvent objects decoded from the
## given byte stream.
static func decode_track(data: PackedByteArray, length: int = -1) -> Array:
	...
	var events: Array = []
	var read_position: int = 0

	while read_position < length:
		var byte: int = data[read_position]
		read_position += 1

		if byte < 0x80:
			# Note event: lead byte is velocity, next byte encodes key + duration.
			if read_position >= length:
				break
			var data_byte: int = data[read_position]
			read_position += 1

			var note_event: NoteEvent = NoteEvent.new()
			note_event.velocity = byte
			note_event.note_byte = data_byte
			note_event.relative_key = data_byte / 19
			var delta_index: int = data_byte % 19
			note_event.delta_time = DELTA_TIME_TABLE[delta_index]

			if note_event.delta_time == 0 and read_position < length:
				note_event.delta_time = data[read_position]
				read_position += 1
			...
			if OPCODE_INFO.has(byte):
				param_count = OPCODE_INFO[byte][1]
			elif EXTRA_OPCODE_PARAM_COUNTS.has(byte):
				param_count = EXTRA_OPCODE_PARAM_COUNTS[byte]

			var opcode_event: OpcodeEvent = OpcodeEvent.new()
			opcode_event.opcode = byte
			opcode_event.params = PackedInt32Array()
			for _param_index: int in param_count:
				if read_position >= length:
					break
				opcode_event.params.append(data[read_position])
				read_position += 1

			events.append(opcode_event)

			if byte == OPCODE_END_BAR:
				break

	return events


static func fft_tempo_to_bpm(tempo_value: int) -> float:
	if tempo_value == 0:
		return 120.0
	return (tempo_value * 256.0) / 218.0
