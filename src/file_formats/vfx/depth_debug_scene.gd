...
## CPU mirror of psx_window_depth() in src/shaders/psx_window_depth.gdshaderinc.
## The numbers this scene prints are only useful if they are the numbers the
## shaders actually write, so the NDC [-1,1] -> window [0,1] conversion has to
## be applied here too. The project renders under gl_compatibility (see
## project.godot renderer/rendering_method), which is the renderer that needs
## the conversion; under Forward+ the shader-side guard skips it.
