...
func get_debug_stats() -> Dictionary:
	# Local fix, 2026-09-13: native counters/voice state are non-atomic and
	# written by the mixer. Snapshot under its lock, not just the SFX scheduler
	# mutex. Callers owning that mutex retain scheduler -> AudioServer order.
	# This serializes streamed rendering; callers of synchronous render_* must
	# still serialize their own producer, as with all other direct SPU access.
	AudioServer.lock()
	var snapshot: Dictionary = _native.get_debug_stats()
	AudioServer.unlock()
	return snapshot
