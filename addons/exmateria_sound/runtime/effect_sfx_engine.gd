...
func _ready() -> void:
	...
	if ae == null:
		...
		return
	# Local modification, 2026-09-13: absent content is normal. Initialize only
	# after the host supplies a valid waveset, including after scene startup.
	if ae.ready_ok:
		_initialize_live()
	else:
		ae.initialized.connect(_initialize_live, CONNECT_ONE_SHOT)


func _initialize_live() -> void:
	if ready_ok:
		return
...
func _rail_stats_locked() -> Dictionary:
	# Caller holds _audio_mutex. SPU.get_debug_stats() additionally takes the
	# mixer lock (scheduler -> AudioServer order) for each native snapshot.
	# Aggregate the native rail counters per unit group.
